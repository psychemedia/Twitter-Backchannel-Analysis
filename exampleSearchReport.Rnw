...
(I've got myself in a muddle around new style and old style RTs... I'm not sure what RT data is being parsed out correctly atm... Maybe it all is...! )
...
A possibly pointless comparison between the number of tweets from a user and the nimber of times they are retweeted (how many of the tweets are retweeted, for example? Lots of them a few times, or one of them lots of times?) This really needs to also convey the number of unique tweets that are retweeted, perhaps using label size?
...
print(p)
@
\caption{Accession of tweeps (ignoring old style RTs)}
\end{center}
\end{figure}
...
\newpage
It's arguably easier to look across a row to spot who's been retweeting a lot of different people. As with the previous chart, the ordering of tweeps on x and y axes doesn't really work for me in this case either. But what would be better?
...
\newpage
\subsection{Tweet and RT Stats}
The following charts are generated from an analysis of the networks of people following hashtag users.

\begin{figure}[htbp]
\begin{center}
<<exampleReachAccessionTweeps, fig = T, echo = F,width=12,height=16>>=
...
p=ggplot(ddj_ncount) + geom_point(aes(x=ttime,y=newuser)) + opts(axis.text.x=theme_text(size=6),axis.text.y=theme_text(size=4))
print(p)
@
\caption{Accession of tweeps}
\end{center}
\end{figure}

To begin with, let's just recap on the accession order of tweeps into their usage of the hashtag (as recorded in the sample period) over time.


\newpage

This chart plots the possible reach of the hashtag, based on the follower counts of each of the hashtag users. The red trace simply sums the follower count of tag users; the blue trace is a count of unique followers. THe green trace is the number of 'repeated' followers, ie folk who follow two or more of the hashtag users.
Note that we could also chart the N-tag user followers, to display folk following N or more of the hashtag users.
\begin{figure}[htbp]
\begin{center}
<<exampleReachCount, fig = T, echo = F>>=

#Plot the cumulative and union flavours of increasing possible audience size, as well as the difference between them
p=ggplot(ddj_ncount) + geom_line(aes(x=ttime,y=count,col='Unique followers')) + geom_line(aes(x=ttime,y=crudetot,col='Cumulative followers')) + geom_line(aes(x=ttime,y=crudetot-count,col='Repeated followers'))
p=p+ labs(colour='Type') + xlab(NULL)
print(p)
@
...
This chart attempts to identify the increased reach achieved over time through new users using the hashtag for the first time.

\begin{figure}[htbp]
\begin{center}
<<exampleReachDelta, fig = T, echo = F>>=

#Number of new unique followers introduced at each time step
p=ggplot(ddj_ncount)+geom_line(aes(x=ttime,y=count-previousCount,col='Actual delta'))

print(p)
@
\caption{Increased reach over time}
\end{center}
\end{figure}

\newpage

The reach improvement by user chart attempts to capture the increase in reach for the tag for each new hashtagger. The total height of the bar represents the follower count for each user. The red segment represents the increased reach for the hashtag (in the sample period) based on that user's follower network. The size of visible blue segment represents the number of followers for each user that may have already seen the hashtag.
...
\begin{figure}[htbp]
\begin{center}
<<exampleReachDeltaByUser, fig = T, echo = F>>=
...
print(p)
@
\caption{Reach proportion by user}
\end{center}
\end{figure}
...
This experimental view represents the "reach duplication quotient" for each user that reports the proportion of their followers that have had a prior opportunity of seeing the hashtag in their stream. One problem with this chart is that is doesn't capture the actual size of a user's follower network, so we donlt get much idea about how many new viewers of the tag each user brings.
...
\begin{figure}[htbp]
\begin{center}
<<exampleReachDuplicationQuotient, fig = T, echo = F,width=16>>=
...
p=ggplot(ddj_ncount) + geom_linerange(aes(x=newuser,ymin=0,ymax=1-(count-previousCount)/userFoCount)) + opts(axis.text.x=theme_text(angle=-90,size=6)) + xlab(NULL)

print(p)
@
\caption{Reach duplication quotient by user}
\end{center}
\end{figure}
...
This counter to the previous view, the "reach offering quotient" for each user that reports the proportion of their followers that have not had a prior opportunity of seeing the hashtag in their stream (that is, it suggests what proportion of a user's followers have not been previously exposed to the tag within the sample period). As before, a problem with this chart is that is doesn't capture the actual size of a user's follower network, so we donlt get much idea about how many new viewers of the tag each user brings.
...
\begin{figure}[htbp]
\begin{center}
<<exampleReachOfferingQuotient, fig = T, echo = F,width=16>>=

p=ggplot(ddj_ncount) + geom_linerange(aes(x=newuser,ymin=0,ymax=(count-previousCount)/userFoCount)) + opts(axis.text.x=theme_text(angle=-90,size=6)) + xlab(NULL)

print(p)
@
\caption{Reach offering quotient by user}
\end{center}
\end{figure}
...
\newpage
This chart tries to capture something of the follower count of each new user, and what proportion of it is new to the hashtag reach audience.
...
\begin{figure}[htbp]
\begin{center}
<<exampleReachImprovmentQuotientScatter, fig = T, echo = F,width=16>>=

#It may be better to plot the ratio as a point with dot size relative to the (log) of the number of followers?
p=ggplot(ddj_ncount) + geom_point(aes(x=newuser,y=(count-previousCount)/userFoCount,size=userFoCount,colour=log(userFoCount))) + opts(axis.text.x=theme_text(angle=-90,size=6)) + xlab(NULL)


print(p)
@
\caption{Reach offering by user}
\end{center}
\end{figure}
...
\newpage
The same chart as before, although with a different colour resolution.
...
\begin{figure}[htbp]
\begin{center}
<<exampleReachImprovmentQuotientScatterColour, fig = T, echo = F,width=16>>=
...
\newpage
The follower hit distribution displays the number of followers (y-axis) against the number of hashtaggers they follow. 

\begin{figure}[htbp]
\begin{center}
<<exampleReachfollowersIndegree, fig = T, echo = F>>=
require(igraph)
...
tmp=sum(g.nodes$Freq)

#We can then chart the result to look at the distribution of how many hashtaggers are followed by how many people
require(ggplot2)
p=ggplot(g.nodes)+geom_linerange(aes(x=Var1,ymin=0,ymax=Freq)) + scale_y_log10() + xlab('In-degree of followers')
print(p)
@
\caption{Follower Hit Distribution}
\end{center}
\end{figure}

\newpage
This chart displays the number of followers (y-axis) who have had the opportunity to see a given number of hashtagged messages in their stream over the sample period. This may relate to several tweets from one person, one tweet from several people, and so on.
Spikes may corresond to users who follow a particular individal with a large reach who has been tweeting a lot...
...
\begin{figure}[htbp]
\begin{center}
<<exampleReachFollowersExposure, fig = T, echo = F,width=16>>=
...
tmp=sum(g.weights$Var1*g.weights$Freq)

#We can also plot the distribution of the number of tagged tweets potentially received by each follower
p=ggplot(g.weights)+geom_linerange(aes(x=Var1,ymin=0,ymax=Freq)) + scale_y_log10() + xlab('Incoming tagged message count')

print(p)
@
\caption{Exposure to tagged tweets}
\end{center}
\end{figure}
\end{landscape}
\SweaveOpts{width=6,height=6}
...
\newpage
This chart gives a view over the distribution of follower counts for hashtag users (binwidth=200).

\begin{figure}[htbp]
\begin{center}
<<exampleFollowerReachDist, fig = T, echo = F,width=16>>=
...
tmp=sum(tagger.nodes$Freq)

#And the distribution of how many followers they have
p=ggplot(tagger.nodes)+geom_histogram(aes(x=Var1,ymin=0,ymax=Freq),binwidth=200)  + xlab('Follower count')
print(p)
@
\caption{Follower count distribution for each hashtagger}
\end{center}
\end{figure}
\end{document}