...
p=ggplot(df.counts)+geom_text(aes(x=fromCount,y=rtbyCount,label=Name),size=2,angle=45)
print(p)
...
p=ggplot(df.counts)+geom_text(aes(x=fromCount,y=rtbyCount,label=Name,size=rtofCount),size=2,angle=45)
print(p)
...
\newpage
If we're interested in folk creating new tweets, we can easily exclude the odl-style RT'd tweets from the chart.
\begin{figure}[htbp]
\begin{center}
<<exampleAccessionNoRT, fig = T, echo = F,width=12,height=16>>=

p=ggplot(subset(df.data,rtt=='T'))+geom_point(aes(x=created,y=screenName,col=rtt))
p=p+opts(axis.text.y=theme_text(size=6))
print(p)
@
...
p=ggplot(subset(df.data,rtt=='RT'))+geom_point(aes(x=created,y=screenName),colour='blue')
p=p+opts(axis.text.y=theme_text(size=6))
print(p)
@
